...
namespace
{
    ...
    std::string so_pin()
    {
        if (const char *env = std::getenv("PKCS11_SO_PIN"); env && *env)
            return env;
        return "1234so";
    }
    ...
    bool login_as(CK_SESSION_HANDLE session, CK_USER_TYPE user_type, const std::string &pin, const std::string &label)
    {
        CK_RV rv = p11->C_Login(
            session,
            user_type,
            reinterpret_cast<CK_UTF8CHAR_PTR>(const_cast<char *>(pin.data())),
            static_cast<CK_ULONG>(pin.size()));
        return expect(label, rv, CKR_OK);
    }

    bool login_or_already(CK_SESSION_HANDLE session, const std::string &pin, const std::string &label)
    {
        CK_RV rv = p11->C_Login(
            session,
            CKU_USER,
        ...
        {
            std::cout << "PASS " << label << " -> " << rv_name(rv) << "\n";
            return true;
        }
        return expect(label, rv, CKR_OK);
    }
    ...
    bool mode_concurrency(const std::string &pin)
    {
        bool ok = true;
        ok &= expect("C_Initialize", p11->C_Initialize(nullptr), CKR_OK);
        ...
        for (int i = 0; i < 8; i++)
        {
            threads.emplace_back([&, i]()
                                 {
            CK_SESSION_HANDLE s = open_session();
            if (!s || !login_or_already(s, pin, "C_Login(CKU_USER/thread)"))
                return;
            CK_MECHANISM mech {CKM_SHA256_RSA_PKCS, nullptr, 0};
            if (p11->C_SignInit(s, &mech, 1) != CKR_OK)
                return;
            CK_BYTE data[] = "test";
            CK_BYTE sig[256];
            CK_ULONG sigLen = sizeof(sig);
            results[i] = p11->C_Sign(s, data, sizeof(data), sig, &sigLen) == CKR_OK; });
        }
    ...
    bool mode_auth(const std::string &pin)
    {
        ...
        ok &= session != 0;
        const std::string admin_pin = so_pin();
        ...
        ok &= expect("C_Login(wrong PIN)",
                     ...
                     CKR_PIN_INCORRECT);
        ok &= expect("C_Login(SO wrong PIN)",
                     p11->C_Login(session, CKU_SO, reinterpret_cast<CK_UTF8CHAR_PTR>(wrong.data()), wrong.size()),
                     CKR_PIN_INCORRECT);
        ok &= login_as(session, CKU_SO, admin_pin, "C_Login(CKU_SO)");
        ok &= expect("C_Logout(SO)", p11->C_Logout(session), CKR_OK);
        ...
        ok &= login(session, pin);
        const std::string rotated_user_pin = pin + "-new";
        ok &= expect("C_SetPIN(user)",
                     p11->C_SetPIN(session,
                                   reinterpret_cast<CK_UTF8CHAR_PTR>(const_cast<char *>(pin.data())),
                                   pin.size(),
                                   reinterpret_cast<CK_UTF8CHAR_PTR>(const_cast<char *>(rotated_user_pin.data())),
                                   rotated_user_pin.size()),
                     CKR_OK);
        ok &= expect("C_Logout(rotated user)", p11->C_Logout(session), CKR_OK);
        ok &= login(session, rotated_user_pin);
        ok &= expect("C_Logout(rotated user 2)", p11->C_Logout(session), CKR_OK);
    ...
    bool mode_key_management(const std::string &pin)
    {
        ...
        ok &= expect("C_GenerateKeyPair", p11->C_GenerateKeyPair(session, &mechanism, pub_template, 3, priv_template, 2, &public_key, &private_key),
                     CKR_OK);
    ...
    bool mode_initialize_token(const std::string &pin)
    {
        ...
        ok &= expect("C_InitToken",
                     p11->C_InitToken(1,
                                      reinterpret_cast<CK_UTF8CHAR_PTR>(const_cast<char *>(so_pin().data())),
                                      so_pin().size(),
                                      label),
                     CKR_OK);

        CK_SESSION_HANDLE session = open_session();
        ok &= session != 0 && login_as(session, CKU_SO, so_pin(), "C_Login(CKU_SO)");
        ok &= expect("C_Finalize", p11->C_Finalize(nullptr), CKR_OK);
        return ok;
    }

    bool mode_bootstrap_default_key(const std::string &pin)
    {
        bool ok = true;
        ok &= expect("C_Initialize", p11->C_Initialize(nullptr), CKR_OK);
        CK_UTF8CHAR label[32];
        ...
        CK_SESSION_HANDLE so_session = open_session();
        ok &= so_session != 0 && login_as(so_session, CKU_SO, so_pin(), "C_Login(CKU_SO)");
        ok &= expect("C_InitPIN",
                     p11->C_InitPIN(so_session,
                                    reinterpret_cast<CK_UTF8CHAR_PTR>(const_cast<char *>(pin.data())),
                                    pin.size()),
                     CKR_OK);
        ok &= expect("C_Logout(SO)", p11->C_Logout(so_session), CKR_OK);

        CK_SESSION_HANDLE user_session = open_session();
        ok &= user_session != 0 && login(user_session, pin);
        CK_MECHANISM mechanism{CKM_RSA_PKCS_KEY_PAIR_GEN, nullptr, 0};
        CK_OBJECT_HANDLE public_key = 0;
        CK_OBJECT_HANDLE private_key = 0;
        unsigned char id_01[] = {0x01};
        const char label_01[] = "ParityKey";
        CK_ULONG bits = 2048;
        CK_ATTRIBUTE pub_template[] = {
            {CKA_ID, id_01, sizeof(id_01)},
            {CKA_LABEL, const_cast<char *>(label_01), sizeof(label_01) - 1},
            {CKA_MODULUS_BITS, &bits, sizeof(bits)},
        };
        CK_ATTRIBUTE priv_template[] = {
            {CKA_ID, id_01, sizeof(id_01)},
        ...
        ok &= expect("C_GenerateKeyPair(default)", p11->C_GenerateKeyPair(user_session, &mechanism, pub_template, 3, priv_template, 2, &public_key, &private_key), CKR_OK);
        ok &= expect("C_Finalize", p11->C_Finalize(nullptr), CKR_OK);
        return ok;
    }
    ...
    bool mode_crypto(const std::string &pin)
    {
        ...
        ok &= expect("C_Encrypt(length)", p11->C_Encrypt(session, reinterpret_cast<CK_BYTE_PTR>(const_cast<char *>(plaintext.data())), plaintext.size(), nullptr, &encrypted_len),
                     CKR_OK);
        std::vector<CK_BYTE> encrypted(encrypted_len);
        ok &= expect("C_Encrypt(data)", p11->C_Encrypt(session, reinterpret_cast<CK_BYTE_PTR>(const_cast<char *>(plaintext.data())), plaintext.size(), encrypted.data(), &encrypted_len),
                     CKR_OK);
        ...
        CK_MECHANISM bad{9999, nullptr, 0};
        expect("Invalid mechanism", p11->C_SignInit(session, &bad, 1), CKR_MECHANISM_INVALID);
        CK_RV rv = p11->C_SignInit(session, &mechanism, 2);
        ok &= expect("C_SignInit(public key)", rv, CKR_KEY_FUNCTION_NOT_PERMITTED);
        ok &= expect("C_Finalize", p11->C_Finalize(nullptr), CKR_OK);
        return ok;
    }

    bool mode_negative(const std::string &pin)
    {
        bool ok = true;
        ok &= expect("C_Initialize", p11->C_Initialize(nullptr), CKR_OK);
        CK_SESSION_HANDLE session = open_session();
        ok &= session != 0 && login(session, pin);
        CK_MECHANISM sign_mech{CKM_SHA256_RSA_PKCS, nullptr, 0};
        ok &= expect("C_SignInit(public)", p11->C_SignInit(session, &sign_mech, 2), CKR_KEY_FUNCTION_NOT_PERMITTED);
        CK_MECHANISM enc_mech{CKM_RSA_PKCS, nullptr, 0};
        ok &= expect("C_DecryptInit(public)", p11->C_DecryptInit(session, &enc_mech, 2), CKR_KEY_FUNCTION_NOT_PERMITTED);
        ok &= expect("C_Finalize", p11->C_Finalize(nullptr), CKR_OK);
        return ok;
    }

    bool mode_compliance(const std::string &pin)
    {
        bool ok = true;
        ok &= expect("C_Initialize", p11->C_Initialize(nullptr), CKR_OK);
        CK_FUNCTION_LIST_PTR_PTR fn_list = nullptr;
        ok &= p11->C_OpenSession != nullptr;
        ok &= p11->C_Login != nullptr;
        ok &= p11->C_GenerateKeyPair != nullptr;
        CK_SESSION_HANDLE session = open_session();
        ok &= session != 0 && login(session, pin);
        CK_MECHANISM_INFO info{};
        ok &= expect("C_GetMechanismInfo(RSA-PKCS)", p11->C_GetMechanismInfo(1, CKM_RSA_PKCS, &info), CKR_OK);
        ok &= info.ulMinKeySize == 2048;
        ok &= expect("C_Finalize", p11->C_Finalize(nullptr), CKR_OK);
        (void)fn_list;
        return ok;
    }

    bool mode_partitions(const std::string &pin)
    {
        bool ok = true;
        ok &= expect("C_Initialize", p11->C_Initialize(nullptr), CKR_OK);

        setenv("PARITY_HSM_PARTITION", "tenant-a", 1);
        CK_SESSION_HANDLE a = open_session();
        ok &= a != 0 && login(a, pin);
        CK_MECHANISM mechanism{CKM_RSA_PKCS_KEY_PAIR_GEN, nullptr, 0};
        CK_OBJECT_HANDLE public_key = 0;
        CK_OBJECT_HANDLE private_key = 0;
        unsigned char id_a[] = {0x02};
        const char label_a[] = "TenantAKey";
        CK_ULONG bits = 2048;
        CK_ATTRIBUTE pub_a[] = {
            {CKA_ID, id_a, sizeof(id_a)},
            {CKA_LABEL, const_cast<char *>(label_a), sizeof(label_a) - 1},
            {CKA_MODULUS_BITS, &bits, sizeof(bits)},
        };
        CK_ATTRIBUTE priv_a[] = {
            {CKA_ID, id_a, sizeof(id_a)},
        ...
        ok &= expect("C_GenerateKeyPair(tenant-a)", p11->C_GenerateKeyPair(a, &mechanism, pub_a, 3, priv_a, 2, &public_key, &private_key), CKR_OK);

        setenv("PARITY_HSM_PARTITION", "tenant-b", 1);
        CK_SESSION_HANDLE b = open_session();
        ok &= b != 0 && login_or_already(b, pin, "C_Login(CKU_USER/tenant-b)");
        unsigned char id_b[] = {0x03};
        const char label_b[] = "TenantBKey";
        CK_ATTRIBUTE pub_b[] = {
            {CKA_ID, id_b, sizeof(id_b)},
            {CKA_LABEL, const_cast<char *>(label_b), sizeof(label_b) - 1},
            {CKA_MODULUS_BITS, &bits, sizeof(bits)},
        };
        CK_ATTRIBUTE priv_b[] = {
            {CKA_ID, id_b, sizeof(id_b)},
        ...
        ok &= expect("C_GenerateKeyPair(tenant-b)", p11->C_GenerateKeyPair(b, &mechanism, pub_b, 3, priv_b, 2, &public_key, &private_key), CKR_OK);

        CK_OBJECT_CLASS cls = CKO_PRIVATE_KEY;
        CK_ATTRIBUTE find_template[] = {{CKA_CLASS, &cls, sizeof(cls)}};
        ok &= expect("C_FindObjectsInit(tenant-b)", p11->C_FindObjectsInit(b, find_template, 1), CKR_OK);
        CK_OBJECT_HANDLE found[4]{};
        CK_ULONG found_count = 0;
        ok &= expect("C_FindObjects(tenant-b)", p11->C_FindObjects(b, found, 4, &found_count), CKR_OK);
        ok &= found_count == 1;
        ok &= expect("C_FindObjectsFinal(tenant-b)", p11->C_FindObjectsFinal(b), CKR_OK);

        setenv("PARITY_HSM_PARTITION", "default", 1);
        CK_SESSION_HANDLE d = open_session();
        ok &= d != 0 && login_or_already(d, pin, "C_Login(CKU_USER/default)");
        ok &= expect("C_FindObjectsInit(default)", p11->C_FindObjectsInit(d, find_template, 1), CKR_OK);
        found_count = 0;
        ok &= expect("C_FindObjects(default)", p11->C_FindObjects(d, found, 4, &found_count), CKR_OK);
        ok &= found_count == 1;
        ok &= expect("C_FindObjectsFinal(default)", p11->C_FindObjectsFinal(d), CKR_OK);
...
int main(int argc, char **argv)
{
    ...
    else if (mode == "bootstrap-default-key")
        ok = mode_bootstrap_default_key(pin);
    else if (mode == "crypto")
        ok = mode_crypto(pin);
    else if (mode == "negative")
        ok = mode_negative(pin);
    else if (mode == "concurrency")
        ok = mode_concurrency(pin);
    else if (mode == "compliance")
        ok = mode_compliance(pin);
    else if (mode == "partitions")
        ok = mode_partitions(pin);
    else
    {
